...
void reader(int reader_id) {
    while (true) {
        ...
        if (read_count == 1) {
            ...
            cout << "Reader " << reader_id << " enters critical section.\n";
        }
        sem_post(&readers_count_sem); // Unlock the read count

        // Reading from shared resource
        cout << "Reader " << reader_id << " is reading the value " << shared_resource << endl;
        sleep(1); // Simulate reading time

        sem_wait(&readers_count_sem); // Lock the read count
        read_count--;
        if (read_count == 0) {
            cout << "Reader " << reader_id << " exits critical section.\n";
...
void writer(int writer_id) {
    while (true) {
        ...
        cout << "Writer " << writer_id << " enters critical section.\n";
        shared_resource++; // Writing to shared resource
        cout << "Writer " << writer_id << " wrote the value " << shared_resource << endl;
        sleep(2); // Simulate writing time
        cout << "Writer " << writer_id << " exits critical section.\n";
